...
{       
 ...
 class BoundElement {
    ...
    valueList? : string[]
    constructor(DOMelement: HTMLElement, objectPath: string, bindMode: string, property :string) {
    ...
    update () {
            ...
            switch (this.bindMode) {
                case ("attr"):
                    if (!this.attr) {throw new Error('No attr value defined for bound element')}
    ...
    let utils = {
        ...
        deepProxy:  function deepProxy (object: any, handler: ProxyHandler<any>) : proxyObservable {
