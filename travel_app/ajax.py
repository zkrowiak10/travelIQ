...
def destination(trip_id):
    ...
    if request.method == "POST":
        try:
            data = request.get_json()
            data = {key: data[key]
                    for key in data if key in models.Destination.__dict__}
            ...
            models.db.session.commit()
            data = {"id": dest.id}
            logging.debug('created trip: ' + str(dest.name) +
                          "with id: " + str(data['id']))
            return jsonify(data), 201

        except Exception as e:
            logging.error(
                "There was an error in loading json POST request: " + str(e))
            # TODO: improve status codes by using better except blocks
            return abort(500)
...
def destinationUpdates(trip_id, dest_id):
    ...
    dest = [dest for dest in currentTrip.destinations if dest.id ==
            int(dest_id)][0]
    if not dest:
        abort(404)
...
def addHotel(trip_id, dest_id):
    ...
    data = request.get_json()
    data = {key: data[key]
            for key in data if key in models.Hotel_Reservation.__dict__}
...
def addRestaurant(trip_id, dest_id):
    ...
    data = request.get_json()
    data = {key: data[key]
            for key in data if key in models.Restaurant.__dict__}
...
def changerRstaurant(trip_id, dest_id, restaurant_id):
    ...
    if not dest:
        abort(404)
    restaurant = [restaurant for restaurant in dest.restaurants if restaurant.id == int(
        restaurant_id)][0]
    if not restaurant:
        abort(404)
